...
func (st *LazySegmentTree) Update2(left, right int, value int) {
	...
	if left <= mid {
		st.lChild.Update2(left, right, value)
	}
	if mid < right {
		st.rChild.Update2(left, right, value)
	}
	st.sum = st.lChild.sum + st.rChild.sum
}

func (st *LazySegmentTree) Add2(left, right int, add int) {
	// 当前节点已被区间 [left, right] 完整覆盖，不再继续递归
	if left <= st.left && st.right <= right {
		st.doAdd(add)
		return
	...
	mid := (st.left + st.right) / 2
	if left <= mid {
		st.lChild.Add2(left, right, add)
	}
	if mid < right {
		st.rChild.Add2(left, right, add)
	}
	st.sum = st.lChild.sum + st.rChild.sum
}
