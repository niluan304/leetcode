...
func stoneGameVI(aliceValues []int, bobValues []int) int {
	...
	valueAlice, valueBob := 0, 0
	for i, p := range pairs {
		if i%2 == 0 {
			valueAlice += aliceValues[p.index]
		} else {
			valueBob += bobValues[p.index]
		}
	}

	return cmp.Compare(valueAlice-valueBob, 0)
}

// 贪心 博弈
//
// # 详细解释为什么要按照 a[i]+b[i] 排序
...
//
// **答**：转换前，我们需要同时考虑 $a[i]$ 和 $b[i]$ 这**两个变量**，不好处理。转换成 Bob 先把所有数字选了，我们就只需要思考 Alice 如何选数字，只有 $c[i]$ 这**一个变量**，更容易处理。从某种程度上来说，这也可以算作一种「正难则反」。
//
// **问**：有没有其它的思考方式？
//
...
// - 空间复杂度：$\mathcal{O}(n)$。
func stoneGameVI2(aliceValues []int, bobValues []int) int {
	var n = len(aliceValues)

	type pair struct{ AliceValue, BobValue int }
	var pairs = make([]pair, n)
	for i := 0; i < n; i++ {
		pairs[i] = pair{
	...
	slices.SortFunc(pairs[:], func(x, y pair) int {
		return (y.AliceValue + y.BobValue) - (x.AliceValue + x.BobValue)
	})
