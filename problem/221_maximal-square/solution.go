...
func maximalSquare(matrix [][]byte) int {
	...
	for r := 0; r < m; r++ {
		for c := 0; c < n; c++ {
			for r1, c1, i := r, c, 1; r1 >= 0 && c1 >= 0; r1, c1, i = r1-1, c1-1, i+1 {
				x := i * i
				if s.query2(r1, c1, r, c) != x {
					break
				}
				ans = max(ans, x)
			}
...
func NewMatrixSum(matrix [][]byte) MatrixSum {
	m, n := len(matrix), len(matrix[0])
	sum := make([][]int, m+1)
	sum[0] = make([]int, n+1)
	for i, row := range matrix {
		...
		for j, x := range row {
			sum[i+1][j+1] = sum[i+1][j] + sum[i][j+1] - sum[i][j] + int(x-'0')
		}
	}
	return sum
}
